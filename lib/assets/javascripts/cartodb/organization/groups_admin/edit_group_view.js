...
module.exports = cdb.core.View.extend({
  ...
  _onClickSave: function (ev) {
    ...
    if (name && name !== this.options.group.get('display_name')) {
      this._setLoading('Guardando cambios');
  ...
  _onClickDelete: function (ev) {
    this.killEvent(ev);
    this._setLoading('Borrando grupo');
  ...
  _showErrors: function (m, res, req) {
    ...
    try {
      ...
      str = 'No se pudo actualizar el grupo por causas desconocidas, por favor inténtalo de nuevo';
    }
