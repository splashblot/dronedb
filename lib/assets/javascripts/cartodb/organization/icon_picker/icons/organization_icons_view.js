...
module.exports = cdb.core.View.extend({
  ...
  _refreshActions: function () {
    ...
    var iconText = (numOfSelectedIcons === 1 ? '1 icono seleccionado' : '' + numOfSelectedIcons + ' iconos seleccionados');
    ...
    if (numOfSelectedIcons > 1) {
      this.$('.js-deleteIcons a').text('Borrar iconos...');
    } else if (numOfSelectedIcons === 1) {
      this.$('.js-deleteIcons a').text('Borrar icono...');
    }
  },
  ...
  _beforeIconDelete: function () {
    ...
    if (this._numOfDeletingProcesses > 0) {
      this._runningMessage = 'Borrando iconos...';
      this.model.set('isProcessRunning', true);
    }
  },
  ...
  _uploadErrorMessage: function (errorText) {
    var message = 'Error subiendo la imagen. ';
    if (errorText) {
      message += '[ ' + errorText + ' ]. ';
    }
  ...
  _deleteErrorMessage: function (errorText) {
    var message = 'Error borrando la imagen. ';
    if (errorText) {
      message += '[ ' + errorText + ' ]. ';
    }
    message += 'Por favor inténtalo de nuevo.';
