  ...
  _maybeDisabledStreetAddresses: function() {
    ...
    if (this._isGoogleMapsEnabled()) {
      if (!isGmeGeocoderEnabled) {
        this._disableTab('Google Maps geocoder is not configured. Please contact us at sales@tileo.co', this._streetAddrTabModel());
      }
    } else {
      if (isGmeGeocoderEnabled) {
        this._disableTab('Geocoder is not configured properly. Please contact us at sales@tileo.co', this._streetAddrTabModel());
      } else if (!this._userGeocoding().hasQuota()) {
        // Credits are only used for users that use non-GME geocoder
        this._disableTab('Your geocoding quota is not defined. Please contact us at sales@tileo.co', this._streetAddrTabModel());
      }
    }
  },
