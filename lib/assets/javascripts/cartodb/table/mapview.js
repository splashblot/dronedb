...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    if (!!this.vis.attributes.tags.includes('raster')) {
      //hardcoded raster hijacking
      //TODO ASAP: move this out to a proper function /cc @apercas

      var raster_vis = this.vis;
      //sanity duplication

      function makeRaster() {
        var map_container = document.querySelectorAll('.panes')[0];
        map_container.innerHTML = "";
        map_container.id = 'raster_map_container';
        map_container.className = 'panes_raster';
        ...
        function currentEndpoint() {
            return `https://${user_data[2]}/user/${user_data[user_data.length-1]}/api/v1/map`;
        }
