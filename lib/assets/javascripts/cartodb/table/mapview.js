...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  setActiveLayer: function(layerView) {
    ...
    if (!!this.vis.attributes.tags.includes('raster')) {
      this.setRasterVis();
    }
    if (!!this.vis.attributes.tags.includes('baselayer')) {
      this.setBaselayerVis();
    }
  },

  setRasterVis: function() {
    //sanity duplication
    var raster_vis = this.vis;

    var makeRaster = function() {
      document.querySelectorAll('.panes')[0].remove();
      var map_container = document.createElement('div');
      map_container.id = 'raster_map_container';
      map_container.style = 'margin-top:80px; height:1000px; display:block;';
      document.body.appendChild(map_container);

      var table_name = raster_vis.related_tables.models[0].attributes.name;
      var center     = raster_vis.map.attributes.center;
      var user_data  = raster_vis.attributes.url.split('/viz/')[0].split('/');

      //create a basic cartomap with the raster query
      var map = new L.Map('raster_map_container', {
          zoomControl: true,
          drawnControl: true,
          center: center,
          zoom: 15
      ...
      function currentEndpoint() {
          return `http://${user_data[2]}/user/${user_data[user_data.length-1]}/api/v1/map`;
      }

      L.tileLayer('http://{s}.tile.opentopomap.org/{z}/{x}/{y}.png', {
          attribution: '<a href="http://cartodb.com">CartoDB</a> | &copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>, <a href="http://viewfinderpanoramas.org">SRTM</a> | Map style: &copy; <a href="https://opentopomap.org">OpenTopoMap</a> (<a href="https://creativecommons.org/licenses/by-sa/3.0/">CC-BY-SA</a>)',
      }).addTo(map);
      ...
      map_container.className = '';
    }
    makeRaster();
  },

  setBaselayerVis: function() {
    //sanity duplication
    var raster_vis = this.vis;
    ...
    var makeVis = function() {
      ...
      $.ajax(`http://${user_data[user_data.length-1]}.${user_data[2]}/api/v2/sql?q=select basemap from ${table_name} where basemap is not null limit 1`).done(function(data) {
        L.tileLayer(data.rows[0].basemap).addTo(map);
      });

      L.tileLayer('http://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png', {
          attribution: '<a href="http://cartodb.com">CartoDB</a> | &copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>, <a href="http://viewfinderpanoramas.org">SRTM</a>',
      }).addTo(map);
