...
var DroneCardPreview = require('../../common/views/dronecard_preview');
...
/**
 * Represents a drone card on dashboard.
 */
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.template = cdb.templates.getTemplate('dashboard/views/drones_item');
  ...
  _renderMapThumbnail: function() {
    ...
    // TODO var previewZoom = (zoom + 2 <= this.model.drone.get("maxZoom")) ? zoom + 2 : zoom;

    var droneCardPreview = new DroneCardPreview({
      el: this.$('.js-header'),
      privacy: this.model.get("privacy"),
      dronesApiResource: cdb.config.getMapsResourceName(ownerUsername),
      username: ownerUsername,
      visId: this.model.get('id')
    });

    if (this.imageURL) {
      droneCardPreview.loadURL(this.imageURL);
    } else {
      droneCardPreview.load();
    }

    droneCardPreview.bind("loaded", function(url) {
      this.imageURL = url;
    }, this);

    this.addView(droneCardPreview);
  },
