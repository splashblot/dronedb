...
module.exports = CoreView.extend({
  ...
  _needsGeocoding: function () {
    if (this.model.attributes.table_name.indexOf('_raster') > 0){ return false;}
  ...
  _getContextMenuOptions: function () {
    ...
    // if not needed anymoer because we can center a raster now 💪
    // if (!this.model.canBeGeoreferenced()) {
  ...
  _centerMap: function () {
    ...
    var query = (nodeModel.get('table_name').indexOf('_raster') > 0) ? nodeModel.get('table_name') : nodeModel.querySchemaModel.get('query');
