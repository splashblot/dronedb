  ...
  _onToggleLayerClicked: function (event) {
    ...
    if (this.model.attributes.table_name.indexOf('_raster') > 0) {
        if (!!this.model.attributes.visible){
            ...
            for (ele in layers) {
	        if (layers[ele].id == this.model.attributes.id){
			vis.map.removeLayerAt(layers.indexOf(layers[ele]) + ((!!vis.map.getBaseLayer().attributes.labels)? 2 : 1));
                        break;
		}
            }
        } else {
	    ...
	    const ENDPOINT= APIURL + "/" + Window.LayerGroupCollection[this.model.attributes.table_name] + "/{z}/{x}/{y}.png?api_key=" + APIKEY;
