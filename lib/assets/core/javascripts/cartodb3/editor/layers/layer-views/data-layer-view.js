  ...
  _onToggleLayerClicked: function (event) {
    ...
    if (this.model.attributes.table_name.indexOf('_raster') > 0) {
		...
		var cleanModifiedLayers = function(id, currentLayerName) {
			id = id || undefined;
			for (ele in layers) {
                		if ((!layers[ele].attributes.visible && 
				    !!layers[ele].attributes.layer_name &&
				    layers[ele].attributes.layer_name.indexOf('_raster')) || 
				    (layers[ele].attributes.urlTemplate && layers[ele].attributes.urlTemplate.indexOf(Window.LayerGroupCollection[currentLayerName]) > 0)) {
					vis.map.removeLayerByCid(layers[ele].cid);
                		}
            		}
			for (ele in layers) {
				if (layers[ele].attributes.visible && 
					...
					const USER    = location.href.split('user/')[1].split('/')[0];
            				...
            				const ENDPOINT= APIURL + "/" + Window.LayerGroupCollection[layers[ele].attributes.layer_name] + "/{z}/{x}/{y}.png?api_key=" + APIKEY;
