...
module.exports = CoreView.extend({
  ...
  _onToggleLayerClicked: function (event) {
    ...
    for (var layer in layers) {
      ...
      lay.attributes.order = (lay.attributes.table_name.indexOf('_raster') > 0) ? 0 : layersLength;
    }
    ...
    if (this.model.attributes.table_name.indexOf('_raster') > 0) {
      ...
      // avoid callback hell (hahahahaha nope)
      $.when( iterateLayers(true) ).done(function(){
		iterateLayers(false);
      });
      // avoid callback hell (hahahahaha nope)
      // clean layers
      // iterateLayers(true, function(){addIteratedLayers});
      // add layers
      // var addIteratedLayers = function(){ iterateLayers(false) };
    }
    this._userActions.saveLayer(this.model, savingOptions);
  },
