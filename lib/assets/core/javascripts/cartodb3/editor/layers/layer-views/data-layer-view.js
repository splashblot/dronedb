...
module.exports = CoreView.extend({
  ...
  _onToggleLayerClicked: function (event) {
    ...
    if (this.model.attributes.table_name.indexOf('_raster') > 0) {
		...
		var cleanModifiedLayers = function(id, currentLayerName) {
			id = id || undefined;
			for (ele in layers) {
				var itelayer = layers[ele];
                		if ((!itelayer.attributes.visible && 
				    !!itelayer.attributes.layer_name &&
				    itelayer.attributes.layer_name.indexOf('_raster')) || 
				    (itelayer.attributes.urlTemplate && itelayer.attributes.urlTemplate.indexOf(Window.LayerGroupCollection[currentLayerName]) > 0)) {
	...
	cleanModifiedLayers(this.model.attributes.id, this.model.attributes.table_name);
    }
