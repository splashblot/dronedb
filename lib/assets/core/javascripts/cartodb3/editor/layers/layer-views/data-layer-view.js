        ...
        for (var ele in layers) {
          ...
          if ((iteIsRaster) ||
            ...
            if (!itelayer.attributes.visible) {
              ...
              if (window.location.href.split('user/').length === 2) {
                ...
                const APIURL = window.location.protocol + '//' + DOMAIN + '/user/' + USER + '/api/v1/map';
                ...
                const USER = window.location.href.split('//')[1].split('/')[0].split('.')[0];
                const DOMAIN = window.location.href.split('//')[1].split('/')[0];
                const APIKEY = vis.attributes.apiKey;
                const APIURL = window.location.protocol + '//' + DOMAIN + '/user/' + USER + '/api/v1/map';
                const ENDPOINT = APIURL + '/' + window.LayerGroupCollection[itelayer.attributes.layer_name] + '/{z}/{x}/{y}.png?api_key=' + APIKEY;
