  ...
  _onToggleLayerClicked: function (event) {
    ...
    if (this.model.attributes.table_name.indexOf('_raster') > 0) {
      ...
      if (!this.model.attributes.visible) {
        ...
        var layer2remove = vis.map.getLayerAt(rasterIndex);
	window.setTimeout(function(){layer2remove.destroy();},0);
        ...
        const ENDPOINT = APIURL + '/' + window.LayerGroupCollection[rasterLayer.getName()] + '/{z}/{x}/{y}.png?api_key=' + APIKEY;
        window.L.tileLayer(ENDPOINT).addTo(vis.map);
      }
    }
    var that = this;
    window.setTimeout(function(){that._userActions.saveLayer(that.model, savingOptions);},1000);
  },
