...
module.exports = CoreView.extend({
  ...
  _onToggleLayerClicked: function (event) {
    ...
    if (this.model.attributes.table_name.indexOf('_raster') > 0) {
	var rasterLayer = vis.map.getLayerById(this.model.id);
          if (!this.model.attributes.visible) {
var rasterIndex = vis.map.layers.models.findIndex(function(item, i){
return item.attributes._url && item.attributes._url.indexOf(window.LayerGroupCollection[rasterLayer.getName()])
});
vis.map.removeLayerAt(rasterIndex + 1)
        } else {
		const CURRENTURI = window.location;
              ...
              const APIKEY = vis.attributes.apiKey;
              const APIURL = CURRENTURI.protocol + '//' + DOMAIN + '/user/' + USER + '/api/v1/map';
              const ENDPOINT = APIURL + '/' + window.LayerGroupCollection[rasterLayer.attributes.layer_name] + '/{z}/{x}/{y}.png?api_key=' + APIKEY;
              window.L.tileLayer(ENDPOINT).addTo(vis.map);
	}
    }
//      var currentLayerName = this.model.attributes.table_name;
//      var iterateLayers = function (remove) {
//        for (var ele in layers) {
//          var itelayer = layers[ele];
//          var iteIsRaster = !!itelayer.attributes.layer_name && itelayer.attributes.layer_name.indexOf('_raster') > 0;
//          if ((iteIsRaster) ||
//            (!!remove && itelayer.attributes.urlTemplate && itelayer.attributes.urlTemplate.indexOf(window.LayerGroupCollection[currentLayerName]) > 0)) {
//            if (!itelayer.attributes.visible) {
//              vis.map.removeLayerByCid(itelayer.cid);
//            } else if (iteIsRaster) {
//              const CURRENTURI = window.location;
//              var USER = '';
//              if (CURRENTURI.href.split('user/').length === 2) {
...
//              const APIKEY = vis.attributes.apiKey;
//              const APIURL = CURRENTURI.protocol + '//' + DOMAIN + '/user/' + USER + '/api/v1/map';
//              const ENDPOINT = APIURL + '/' + window.LayerGroupCollection[itelayer.attributes.layer_name] + '/{z}/{x}/{y}.png?api_key=' + APIKEY;
//              window.L.tileLayer(ENDPOINT).addTo(vis.map);
//            }
//          }
//        }
