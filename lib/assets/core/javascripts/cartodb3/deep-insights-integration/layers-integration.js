...
var LAYER_TYPE_TO_LAYER_CREATE_METHOD = {
  ...
  'wms': 'createWMSLayer',
  'raster_tileo': 'createRasterTileoLayer'
};
...
var LayersIntegration = {
  ...
  _createLayer: function (layerDefModel, options) {
    ...
    attrs.type = (attrs.layer_name != null && attrs.layer_name.indexOf('_raster') >= 0) ? 'raster_tileo' : attrs.type;
  ...
  _setLayerError: function (layerDefinitionModel, cdbError) {
    // Check error is not thrown because of RASTER + the_geom_webmercator
    if (!!(layerDefinitionModel.attributes.table_name.indexOf('raster') > 0) && !!(cdbError.message.indexOf('column "the_geom_webmercator" does not exist') > 0)) {
      return;
    }
