...
var THE_GEOM_WEBMERCATOR_NOT_FOUND_ERROR = 'column \"the_geom_webmercator\" does not exist';
...
module.exports = Backbone.Model.extend({
    ...
    opts.error = function (model, response) {
      if (response && response.statusText !== 'abort') {
        ...
        if (error.length === 1 && error[0] === THE_GEOM_NOT_FOUND_ERROR) {
          ...
          this.fetch(_.extend({}, opts, { geomColumnName: 'the_raster_webmercator' }));
        }
        else {
  ...
  _getSqlApiQueryParam: function (geomColumnName) {
    if (geomColumnName == 'the_raster_webmercator') {
      return this.get('query');
    }
